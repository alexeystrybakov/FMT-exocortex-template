...
```bash
for repo in $(ls /home/alexey/IWE/); do
  if [ -d /home/alexey/IWE/$repo/.git ]; then
    commits=$(git -C /home/alexey/IWE/$repo log --since="today 00:00" --oneline --no-merges 2>/dev/null \
...
grep -nE "→ ждёт|ждёт|dep:|блокер|blocked:|остановлен|ждёт согласования" \
  /home/alexey/.claude/projects/*/memory/MEMORY.md 2>/dev/null
```
...
> **Условный шаг:** скрипт опционален. Если `/home/alexey/IWE/{{GOVERNANCE_REPO}}/scripts/check-index-health.py` отсутствует — пропустить.

```bash
SCRIPT="/home/alexey/IWE/{{GOVERNANCE_REPO}}/scripts/check-index-health.py"
...
```bash
TODAY=$(date +%Y-%m-%d)
grep -l "Итоги дня" /home/alexey/IWE/{{GOVERNANCE_REPO}}/archive/day-plans/DayPlan\ ${TODAY}.md 2>/dev/null \
...
( grep -rl "Итоги.*${DAY_NUM}" /home/alexey/IWE/{{GOVERNANCE_REPO}}/current/WeekReport\ W*.md 2>/dev/null \
  || grep -rl "Итоги.*${DAY_NUM}" /home/alexey/IWE/{{GOVERNANCE_REPO}}/current/WeekPlan\ W*.md 2>/dev/null ) \
