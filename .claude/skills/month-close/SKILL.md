...
for repo in $(ls /home/alexey/IWE/); do
  if [ -d /home/alexey/IWE/$repo/.git ]; then
    count=$(git -C /home/alexey/IWE/$repo log --since="$MONTH_START" --until="$MONTH_END" --oneline --no-merges 2>/dev/null | wc -l)
...
```bash
cd /home/alexey/IWE/{{GOVERNANCE_REPO}} && git add -A && git commit -m "month-close: close YYYY-MM"
```
