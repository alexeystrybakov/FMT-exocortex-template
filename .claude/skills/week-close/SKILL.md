...
for repo in $(ls /home/alexey/IWE/); do
  if [ -d /home/alexey/IWE/$repo/.git ]; then
    commits=$(git -C /home/alexey/IWE/$repo log --since="last monday 00:00" --until="today 00:00" --oneline --no-merges 2>/dev/null)
...
**Календарь недели:**
```bash
bash /home/alexey/IWE/scripts/server-calendar.sh --week $(date -v-mon +%Y-%m-%d 2>/dev/null || date -d "last monday" +%Y-%m-%d)
```
...
```bash
echo "=== distinctions.md ===" && wc -l /home/alexey/IWE/.claude/rules/distinctions.md
...
```bash
cd /home/alexey/IWE/{{GOVERNANCE_REPO}} && git add -A && git commit -m "week-close: W{N} итоги q:{score}" && git push
```
