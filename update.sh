...
# After merge, new lines from upstream may contain /home/alexey/IWE etc.
# Detect values from the current environment
PLACEHOLDER_COUNT=$(grep -r '/home/alexey/IWE' "$EXOCORTEX_DIR" --include="*.md" --include="*.sh" --include="*.json" --include="*.yaml" --include="*.yml" --include="*.plist" -l 2>/dev/null | wc -l | tr -d ' ')

if [ "$PLACEHOLDER_COUNT" -gt 0 ]; then
    echo "  Found $PLACEHOLDER_COUNT files with unsubstituted /home/alexey/IWE"
    find "$EXOCORTEX_DIR" -type f \( -name "*.md" -o -name "*.json" -o -name "*.sh" -o -name "*.plist" -o -name "*.yaml" -o -name "*.yml" \) | while read file; do
        sed -i '' "s|/home/alexey/IWE|$WORKSPACE_DIR|g" "$file"
    done
    echo "  Re-substituted /home/alexey/IWE → $WORKSPACE_DIR"
