...
find "$TEMPLATE_DIR" -type f \( -name "*.md" -o -name "*.json" -o -name "*.sh" -o -name "*.plist" -o -name "*.yaml" -o -name "*.yml" \) | while read file; do
    sed -i \
        -e "s|alexeystrybakov|$GITHUB_USER|g" \
        -e "s|/home/alexey/IWE|$WORKSPACE_DIR|g" \
        -e "s|/home/alexey/.npm-global/bin/claude|$CLAUDE_PATH|g" \
        -e "s|-home-alexey-IWE|$CLAUDE_PROJECT_SLUG|g" \
        -e "s|2|$TIMEZONE_HOUR|g" \
        -e "s|7:00 MSK|$TIMEZONE_DESC|g" \
        -e "s|/home/alexey|$HOME_DIR|g" \
        "$file"
done
