...
if [ ! -d "$DS_DIR/.git" ]; then
    ...
    # (1) /home/alexey/IWE/FMT-strategy-template/ — отдельная директория (авторская)
    # (2) /home/alexey/IWE/FMT-exocortex-template/templates/strategy-skeleton/ — внутри FMT (приезжает через update.sh)
...
"
    ...
    for target in $LEAK_TARGETS; do
        ...
        HITS=$(grep -rIl --include='*.sh' --include='*.env' --include='*.service' \
            ...
            -e "/home/alexey" -e "{{CLAUDE_PROJECT_SLUG}}" \
    ...
    if [ $LEAK_COUNT -gt 0 ]; then
        echo "**Найдено $LEAK_COUNT файлов с macOS-путями.** Решение: заменить на \`\$HOME\`, \`/home/alexey\` или \`\$IWE_ROOT\`. Возможные молчаливые сбои в скриптах."
        OPTIONAL_MISSING=$((OPTIONAL_MISSING + 1))
    else
