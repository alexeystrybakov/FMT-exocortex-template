...
3. Заменяет 7 плейсхолдеров (`alexeystrybakov`, `/home/alexey/IWE` и др.)
...
1. Замените `alexeystrybakov` на ваш GitHub username во всех файлах
2. Замените `/home/alexey/IWE` на путь к рабочей директории (напр. `~/IWE`)
3. Замените `/home/alexey` на домашнюю директорию (значение `$HOME`)
4. Замените `-home-alexey-IWE` на путь через дефисы (напр. для `~/IWE` → `-Users-yourname-IWE`)
5. Замените `2` на час запуска стратега в UTC (напр. `4` для 7:00 MSK)
6. Замените `7:00 MSK` на описание времени (напр. `7:00 MSK`)
7. Замените `/home/alexey/.npm-global/bin/claude` на путь к Claude CLI (напр. `/opt/homebrew/bin/claude`)
